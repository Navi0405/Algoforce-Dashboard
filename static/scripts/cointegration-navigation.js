export async function updateNavigationData1(navigationData) {
    try {
            ...
            $("#your_file-unrealized-pnl").text(newPrice).css('color', getColorAndArrow(unrealizedPNL).color);
            
            // Arrow for total balance
            // let arrowBalance = previousTotalBalance !== null && totalBalance > previousTotalBalance ? '↑' : '↓';
            $("#your_file-total-balance").text(roundedTotalBalance);

            // Update return metrics
            let colorAndArrowReturn = getColorAndArrow(totalReturn);
            $("#your_file-return").text(roundedAccountReturn).css('color', colorAndArrowReturn.color).append(" " + colorAndArrowReturn.arrow);

            // Update realized PnL
            if (profitDollarData < 0) {
                $(`#your_file-total-realizedPnl`).text('-$' + Math.abs(profitDollarData.toFixed(2))).css('color', getColorAndArrow(profitDollarData).color).append(" " + getColorAndArrow(profitDollarData).arrow);
            } else {
                $(`#your_file-total-realizedPnl`).text('$' + profitDollarData.toFixed(2)).css('color', getColorAndArrow(profitDollarData).color).append(" " + getColorAndArrow(profitDollarData).arrow);
            }
            $("#your_file-unrealized-pnl").data("lastprice", parseFloat(unrealizedPNL));
        ...
        // Handle the error, you can show a message to the your/ or retry the request
    }
}
...
export async function updateNavigationData2(navigationData) {
    ...
    try {
            ...
            // }
            $(`#your_file-trading-days-value`).text(total_trading_days);
            $(`#your_file-winning-days`).text(total_winning_days).append(" " + getColorAndArrow(total_winning_days).arrow);
            $(`#your_file-total-trades`).text(total_trades);
            ...
            $("#your_file-win-rate").text(total_winning_rate.toFixed(2)+'%').append(" " + getColorAndArrow(total_winning_rate).arrow);
            if (max_draw_down_sum < 0){
                $('#your_file-max-draw-down').text('-' + Math.abs(max_draw_down_sum.toFixed(3)) + '%');

            }
            else {
                $(`#your_file-max-draw-down`).text( (max_draw_down_sum.toFixed(3) ) + '%');
            }
            if (total_fees < 0){
                $(`#your_file-total-fees`).text('-$' + total_fees.toFixed(2));
            }
            else {
                $(`#your_file-total-fees`).text('-$' + total_fees.toFixed(2));
            }
