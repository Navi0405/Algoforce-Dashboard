...
export async function hideSpinner() {
    ...
    console.log('Spinner visibility after hide:', fullPageSpinner.is(':visible')); // Check visibility
        ...
        function updateAllAccountsFlag() {
            ...
            // Check if only your_account is checked
            if (checkedValues.length === 1 && checkedValues.includes('your_account')) {
    ...
    $(".resbat").click(async function() {
        ...
        try {
            ...
            if (selectedFilter === "seven") {
                response = await $.ajax({
                    url: `/api/date_pages?date_range=last_7_days&your_account_accounts=${checkedValues}`,
                    type: 'GET'
                });
            } else if (selectedFilter === "three") {
                response = await $.ajax({
                    url: `/api/date_pages?date_range=last_3_months&your_account_accounts=${checkedValues}`,
                    type: 'GET'
                });
            } else if (selectedFilter === "one") {
                response = await $.ajax({
                    url: `/api/date_pages?date_range=last_30_days&your_account_accounts=${checkedValues}`,
                    type: 'GET'
                });
            } else if (selectedFilter === "year") {
                response = await $.ajax({
                    url: `/api/date_pages?date_range=last_1_year&your_account_accounts=${checkedValues}`,
                    type: 'GET'
                });
            }
            // Handle response
        ...
        $("#date_filter").on('click', async function() {
            ...
            let checkedCoins = [];
            let coins_list = 'UNIUSDT,AVAXUSDT,LINKUSDT,XRPUSDT,ETHUSDT,BNBUSDT,SOLUSDT,symbol,LTCUSDT,MATICUSDT,ADAUSDT,BTCUSDT,DOTUSDT';
            ...
            try {
                const response = await $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        your_account_accounts: checkedAccountsStr,
