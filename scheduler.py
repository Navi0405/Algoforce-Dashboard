...
#     script_path = r"..\file/\storage\cloud_storage.py"
...
def sql_live_data_scheduler():
    script_path = r"..path/\to/\your/\your/\jonathan-dashboard\PINAKA_LATEST\file/\your_dashboard\sql_live_data_3.py"
    # print(f"Running script: {script_path}")
...
def binance_data_api_scheduler():
    script_path = r"..path/\to/\your/\your/\jonathan-dashboard\PINAKA_LATEST\file/\your_dashboard\binance_data.py"
    # print(f"Running script: {script_path}")
    result = subprocess.run(['python', script_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    if result.returncode == 0:
        print("Script ran successfully.")
