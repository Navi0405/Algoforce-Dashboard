...
import time 
import os
# from your_dashboard.binance_data import *
import urllib.request
import pyfiglet
import csv
# from scheduler import url

engine = create_engine('mysql+mysqlconnector://root:password@localhost/your_dashboard')

your_accountxaccount = ["your_account", "your_account","your_account","your_account", "your_account","your_account"]

# your_accountxaccount = ["your_account", "your_account"]
...
def access_trades_live(acc_name):
    ...
    live_trades_folder = os.path.join(path, r'..path/\to/\your/\your/\your/\file/\your_dashboard\your_account/')
...
def read_csv_and_insert_to_mysql(conn, csv_file_path, your_accountxaccount):
    def read_csv_and_insert_to_mysql_thread(conn, csv_file_path, your_accountxaccount):
        try:
            cursor = conn.cursor()

            # Truncate table before inserting new data
            cursor.execute(f"TRUNCATE TABLE {your_accountxaccount}")
            ...
            with open(csv_file_path, 'r', newline='') as file:
                ...
                for row in csv_reader:
                    try:
                        ...
                        sql = f"""
                            INSERT IGNORE INTO {your_accountxaccount} (date, symbol, id, orderId, side, price, qty, realizedPnl, marginAsset, 
    ...
    t = threading.Thread(target=read_csv_and_insert_to_mysql_thread, args=(conn, csv_file_path, your_accountxaccount))
    t.start()
    t.join()
...
def main():
    ...
    if conn:
        ...
        for table in your_accountxaccount:
            lower_table = table.lower()  # Convert the your_accountxaccount table name to lowercase for comparison
            if lower_table in tables:
                ...
                if os.path.isfile(csv_file_path):
                    ...
                    ascii_banner = pyfiglet.figlet_format("your_name DASHBOARD!!")
                    print(ascii_banner)
