...
clients = {account: get_client(account) for account in your_accountxaccount}
...
def access_trades_live(initial_your_accountx):
    path = os.getcwd()
    filename_trades = f"{initial_your_accountx}_4H_continuing_trades.csv"
    live_trades_folder = os.path.join(path, 'your_account/')
...
def update_csv():
    files = [access_trades_live(account) for account in your_accountxaccount]
...
os.chdir(base_dir)
# update_csv()
for initial_your_accountx in tqdm(your_accountxaccount, desc='Processing to SQL'):
    conn = connect_to_mysql()
    
    csv_file_path = os.path.join(
        r'..path/\to/\your/\your/\your/\file/\your_dashboard\your_account/', 
        f'{initial_your_accountx}_4H_continuing_trades.csv'
    )
    ...
    read_csv_and_insert_to_mysql(conn, csv_file_path, initial_your_accountx)
    print("done", initial_your_accountx)
    ...
    # winning_rate_per_account(conn, initial_your_accountx)  # Uncomment if this function is required
