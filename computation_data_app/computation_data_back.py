...
from your_dashboard.computation_data_app.cache_manager import *
...
async def metrics(accounts):
    ...
    organized_results["total_unrealized_pnl_data"]["your_account"] = total_unrealized_pnl_total
    organized_results["total_return_data"]["your_account"] = total_return_total
    organized_results["total_balance_data"]["your_account"] = total_balance_total
    organized_results["margin_total_data"]["your_account"] = margin_total_total
...
async def data_computations(cached_data, start_date, end_date, your_account_accounts, calculation_types):
    ...
    try:
        ...
        for your_account_account in your_account_accounts:
            api_data = [entry for entry in cached_data if entry.get('your_accountx_account') == your_account_account]
            ...
            for calculation_type in calculation_types:
                if calculation_type in calculation_functions:
                    if calculation_type == 'max_draw':
                        results[calculation_type] = await calculation_functions[calculation_type](filtered_df, your_account_account)
                    else:
                        results[calculation_type] = calculation_functions[calculation_type](filtered_df)

            computation_results = await computation2(api_data, your_account_account, start_date)
            results.update(computation_results)

            results_per_account[your_account_account] = results
...
async def calculate_max_draw(filtered_df, your_account_account):
    try:
        metrics = await asyncio.to_thread(account_metrics, your_account_account)
...
# results = compute_data(cached_data, '2024-01-01', '2024-04-01', your_account_account, calculation_types)
...
async def computation2(api_data, account_name, start_date):
    ...
    # your_account_winrates_data = {}

    # Calculating winrate per your_account account
    your_account_winrates = df.groupby('your_accountx_account')['win/lose'].mean() * 100
    your_account_winrates = your_account_winrates.round(2).to_dict()

    # Getting total trades per your_account account
    your_account_total_trades = df.groupby('your_accountx_account')['realizedPnl'].count().to_dict()

    # Printing winrates and total trades per your_account account
    for your_account_account, winrate in your_account_winrates.items():
        # print(f"{your_account_account}: winrate data value: {winrate}")
        pass

        if your_account_account in your_account_total_trades:
            total_trades = your_account_total_trades[your_account_account]
            # print(f"{your_account_account}: total_trades data value: {total_trades}")
            pass

        else:
            print(f"{your_account_account}: total_trades data value: 0 (no trades)")



    overall_data = {"overall": overall, "trades": trades, "winrate_per_symbol": winrates, "your_account_winrates": your_account_winrates, 'total_trades_accounts':your_account_total_trades}
    print("\n\n\n", overall_data,'\n\n\n')
...
your_account_account = input("PLEASE INPUT account: ") or None

print(your_account_account)


async def main():
    cached_data = await fetch_get_data()
    result = await data_computations(cached_data, start_date, end_date, your_account_account, calculation_types)
    # print(cached_data)
...
#     # task2 = asyncio.create_task(computation2(your_account_account, start_date))
#     # result = await task2
#     # print(result)
...
#     # winning rate for your_accountaccounts
#     winning_rate_per_account_dict = result.get('your_account_winrates')
