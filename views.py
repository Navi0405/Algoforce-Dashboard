...
from .computation_data_app.your_file_computation_test import *
...
async def pages(request):
    ...
    elif load_template == 'your_file-dashboard':
        template_path = 'home/your_file-dashboard.html'
...
your_accountx_models = [your_account, your_account, your_account, your_account, your_account, your_account, your_account]
your_accountx_accounts = None
ph_timezone = pytz.timezone('Asia/Manila')
# Get the current date in the specified time zone
presentdate_ph = datetime.now(ph_timezone).date()
# Calculate the date 7 days ago in the specified time zone
last_7_days_ph = presentdate_ph - timedelta(weeks=7)
...
#     filename = f"your_accountx_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
async def get_data(request):
    try:
        ...
        async def fetch_data_from_model(your_accountx_model):
            return await asyncio.to_thread(list, your_accountx_model.objects.all().values())
        
        # Fetch data from all your_accountx models concurrently
        tasks = [asyncio.create_task(fetch_data_from_model(model)) for model in your_accountx_models]
        your_accountx_data_list = await asyncio.gather(*tasks)
        
        # Process the fetched data
        for your_accountx_model, your_accountx_data in zip(your_accountx_models, your_accountx_data_list):
            sorted_your_accountx_data = sorted(your_accountx_data, key=lambda x: x["date"])  # Sort by date or any other key as required
            for entry in sorted_your_accountx_data:
                entry_with_account = {
                    "your_accountx_account": your_accountx_model.__name__,
                    "date": entry["date"],
                    "symbol": entry["symbol"],
                    "id": entry["id"],
        ...
        filename = r'..\file/\your_dashboard\computation_data_app\historical_klines.csv'
...
async def get_all_data_view(request):
    try:
        ...
        filtered_data = [
            {
                "your_accountx_account": item.get("your_accountx_account", ""),
                "date": item.get("date", ""),
...
#for filtering the your_account account


#for datefiltering
# lock = asyncio.Lock()
@api.get("/your_accountxaccount")
async def your_accountxaccount(request):
    # your_account_accounts: str = Form(your_accountx_accounts)):
    # async with lock:
    global your_accountx_accounts
    print(your_accountx_accounts)

from collections import defaultdict
@api.get("/initial_load")
async def get_initial_load(request):
    global start_date_global, end_date_global
    your_account_accounts = None
    symbols = None
    ...
    result = await asyncio.create_task(data_computations(cached_data, start_date_global, end_date_global, your_account_accounts, calculation_types, symbols))
    ...
    your_account_winrates = {k: float(v) for k, v in result.get("your_account_winrates", {}).items()}
    profits_by_coin = {k: float(v) for k, v in result.get("profits_by_coin", {}).items()}
    your_account_total_trades_count = {k: float(v) for k, v in result.get("your_account_total_trades_count", {}).items()}

    your_accountx_all_accounts = ['your_account', 'your_account', 'your_account', 'your_account', 'your_account', 'your_account']
    
    # Convert start and end dates to aware datetimes with UTC timezone
    start_date_dt = datetime.strptime(start_date_global, "%Y-%m-%d").replace(tzinfo=timezone.utc)
    end_date_dt = datetime.strptime(end_date_global, "%Y-%m-%d").replace(tzinfo=timezone.utc)

    # Step 1: Filter the Data
    filtered_data = [
        item for item in cached_data
        if start_date_dt <= item["date"] <= end_date_dt  # Directly compare aware datetime objects
        and item["your_accountx_account"] in your_accountx_all_accounts
    ...
    # Step 2: Aggregate the Data with Date-Time and Symbol
    aggregated_data = defaultdict(lambda: {'realizedPnl': 0, 'date': None, 'symbol': None, 'your_accountx_account': 'your_account'})
    for item in filtered_data:
        datetime_obj = item['date']  # This is already an aware datetime object

        # Use the full datetime (including time) as the key
        key = (datetime_obj, item['symbol'])
        aggregated_data[key]['date'] = datetime_obj.isoformat()  # Keep date-time in ISO format
        aggregated_data[key]['symbol'] = item['symbol']
        aggregated_data[key]['realizedPnl'] += item['realizedPnl']

    # Convert the aggregated data to a list and sort it
    ...
    # Step 3: Merge the aggregated data with filtered_data
    filtered_data.extend(merged_results)  # Merging aggregated data into filtered_data
    ...
    metrics_2 = await asyncio.create_task(all_metrics_json_data_entry_data_only(start_date_global, end_date_global))

    json_data = {
        "filtered_data": filtered_data,  # Updated with merged data
        "response_data": response_data,
        "symbols_winrate": winrates,
        "winning_rate_acc": your_account_winrates,
        "overall": overall,
        ...
        "your_account_total_trades_count": your_account_total_trades_count,
        "all_metrics_json_data_entry_data_only": metrics_2
    }

    cache["/api/initial_load"] = json_data

    data_json = orjson.dumps(json_data)
...
async def post_filtered(request,
                        your_account_accounts: str = Form(''),
                        start_date: str = Form(start_date_global),
                        end_date: str = Form(end_date_global),
                        symbols: str = Form(None)):

    try:
        global start_date_global, end_date_global, your_accountx_accounts

        start_date_global = start_date
        end_date_global = end_date
        symbols = symbols.split(',') if symbols else None

        # Handle empty your_account_accounts
        your_account_accounts = your_account_accounts.strip()
        if your_account_accounts:
            your_account_accounts_list = your_account_accounts.split(',')
        else:
            your_account_accounts_list = []

        is_your_account = "your_account" in your_account_accounts_list
        ...
        filtered_data = [
            {
                "your_accountx_account": item["your_accountx_account"],
            ...
            and (not your_account_accounts_list or item["your_accountx_account"] in your_account_accounts_list)
            and (not symbols or item["symbol"] in symbols)
        ]
        ...
        if is_your_account:
            # Aggregate data for all your_account accounts and treat it as "all_account"
            your_accountx_all_accounts = ['your_account', 'your_account', 'your_account', 'your_account', 'your_account', 'your_account']
            ...
            aggregated_data_all_account = defaultdict(lambda: {'your_accountx_account': 'your_account', 'date': None, 'realizedPnl': 0, 'symbol': None})
        ...
        merged_results = sorted(
            combined_data,
            key=lambda x: (x['date'], x['symbol'], x['your_accountx_account'])
        )
...
async def get_pagination(
    ...
    your_account_accounts: str = Query("")
    ...
    your_accountx_accounts = your_account_accounts


    print("your_account_accounts", your_account_accounts)
    ...
    try:
        ...
        filtered_data = [
            ...
            for item in cached_data
            if start_date <= item["date"].date() <= end_date
            and (not your_account_accounts or item["your_accountx_account"] in your_account_accounts)
            # and (not symbols or item["symbol"] in symbols)
        ]
        ...
        # profit_dollar = await asyncio.to_thread(print_profits, clients, initial_balance, your_account_accounts)
...
,
        ...
        your_account_winrates = result.get("your_account_winrates", {})
        ...
        your_account_winrates = {k: float(v) for k, v in your_account_winrates.items()}
...
async def get_Coint_data(request):
    """View to return trading data for your_account and your_account account."""
    try:
...
async def print_report(request):
    ...
    your_account_total_trades_count = json_data.get("your_account_total_trades_count", {})
    ...
    # print("Winning Rate Ac..path/", winning_rate_acc)
    ...
    # Specifically extract your_account value if it exists
    # your_account_total_starting = all_metrics_json_data_entry_data_only.get("your_account")
    # if your_account_total_starting is not None:
    #     renamed_overall["Starting Balance"] = your_account_total_starting
    ...
    if winning_rate_acc and your_account_total_trades_count and all_metrics_json_data_entry_data_only and account_metrics:
        ...
        your_account_values = [f"{your_account_total_trades_count.get(account, 'N/A')}" if isinstance(your_account_total_trades_count.get(account), int) else str(your_account_total_trades_count.get(account)) for account in winning_rate_acc.keys()]
        your_account_starting_balance = [
        ...
        winning_rate_data = [
            ...
            ['Total Trades'] + your_account_values,
            ['Starting Balance'] + your_account_starting_balance,
            ['Current Balance'] + your_account_current_balance
        ]
        print("account_headers", account_headers)

    else:
        # Check if there are any accounts to process
        if winning_rate_acc or your_account_total_trades_count or all_metrics_json_data_entry_data_only or account_metrics:
            account_headers = [account_rename_map.get(acc, acc) for acc in winning_rate_acc.keys()]
            ...
            winning_rate_data = [['Account', 'Winning Rate', 'your_account Total Trades Profit', 'your_account Starting Balance', 'your_account Current Balance']]
...
#     your_account_accounts = cache.get("/api/your_accountxaccount")
...
#         await asyncio.sleep(0.01)    
#         task2 = asyncio.create_task(computation2(your_account_accounts, start_date_global))
#         result2 = await task2
